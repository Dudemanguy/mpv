...
``--hwdec=<api1,api2,...|no|auto|auto-copy>``
    ...
    .. note::
        ...
        possible hwdec, but if ``auto-safe`` didn't work, you will probably need
        to know exactly which hwdec matches your hardware and read up on that
        entry below.

        If ``auto-safe`` produced the desired results, we recommend just
    ...
    :auto:        exactly the same as ``auto-safe``
    ...
    ``auto-safe`` tries to automatically enable hardware decoding using the
    ...
    ``auto-unsafe`` is similar to ``auto-safe``, but without the whitelist.
    ...
    ``auto-copy-unsafe`` is the same as ``auto-copy`` except that it go through
